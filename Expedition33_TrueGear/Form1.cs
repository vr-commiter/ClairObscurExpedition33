...
namespace Expedition33_TrueGear
{
    public partial class Form1: Form
    {
        private static string steamAppID = "1903340";
        private static string inertFilePath = "\\Sandfall\\Binaries\\Win64\\TrueGear.log";
        ...
        public const string STEAM_OPENURL = "steam://rungameid/1903340";
        ...
        private static string appName = "Expedition33*.exe";
            ...
            if (!string.IsNullOrEmpty(lastUsedPath))
            {
                if (CheckPath(lastUsedPath))
                {
                    ...
                    else
                    {
                        try
                        {
                            var matchingFiles = Directory.GetFiles(_selectedPath, appName);

                            if (matchingFiles.Length > 0)
                            {
                                string exePath = matchingFiles[0]; // 取第一个匹配的文件
                            ...
                            {
                                Console.WriteLine("未找到匹配的 Expedition33*.exe 文件！");
                            }
                            //var processInfo = new ProcessStartInfo
                            //{
        ...
        private bool CheckPath(string tmpSelectedPath)
        {
            try
            {
                string[] matchingFiles = Directory.GetFiles(tmpSelectedPath, "Expedition33*.exe");
                if (matchingFiles.Length > 0)
                {
                    // 取第一个匹配的文件（或按需选择）
                    string matchedExePath = matchingFiles[0];
        ...
        private static void GameEventCheck(string gameEvent)
        {
            ...
            Console.WriteLine($"Play :{gameEvent}");
        }
